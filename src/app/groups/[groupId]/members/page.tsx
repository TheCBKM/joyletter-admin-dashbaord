...
export default async function GroupMembersPage({ params }: PageProps) {
  ...
  const memberError = membersResult.status === "rejected" ? "Unable to load members right now." : null;
    ...
    <div className="space-y-8">
      <div className="glass-panel flex flex-wrap items-center justify-between gap-4 rounded-4xl px-6 py-7 lg:px-10 lg:py-10">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-sm text-brand-900/70">
            Manage the roster for <span className="font-semibold text-brand-700">{group.name}</span> and add new participants.
          </p>
        </div>
      ...
      <section className="space-y-4">
        <header>
          ...
          <p className="text-sm text-brand-900/70">
            Invite someone new to this group. They will appear in the table below after creation.
          </p>
        </header>
      ...
      <section className="space-y-4">
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-xl font-semibold text-brand-900">Member directory</h2>
            <p className="text-sm text-brand-900/70">Review contact details for every member.</p>
          </div>
        ...
        {memberError ? (
          <p className="rounded-2xl bg-red-100 px-4 py-3 text-sm text-red-700">{memberError}</p>
        ) : (
